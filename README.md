...
**⚠️ 注意**：`third_party/vec0.dll` **是必需的**。本库专门用于 sqlite-vec 模式，如果扩展加载失败，程序会直接报错退出，不会回退到普通 SQL 实现。
...
运行后会生成 `.db` 文件。**注意**：必须使用支持 sqlite-vec 扩展的工具查看，普通 SQLite 客户端无法识别 vec0 虚拟表。
